...
function Row(props) {
  ...
  const handleReqChange = (prop) => async () => {
    console.log(prop);
    const result = await axios.post('/api/admin/changeReq', { type: prop.type, user: prop.user });
    console.log(result);
    ...
    <React.Fragment>
      <TableRow sx={{ '& > *': { borderBottom: 'unset' } }}>
        ...
        <TableCell align="center">
          <Button
            variant="contained"
            onClick={handleReqChange({ type: 'accept', user: row.user[0] })}
          >
            Accept
          </Button>
        </TableCell>
        <TableCell align="center">
          <Button
            variant="contained"
            onClick={handleReqChange({ type: 'decline', user: row.user[0] })}
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{ margin: 1 }}>
              ...
              <Table size="small" aria-label="purchases">
                <TableHead>
                  <TableRow>
                    <TableCell align="center" style={{ fontSize: '16px', color: '#014988' }}>
                      First Name
                    </TableCell>
                    <TableCell align="center" style={{ fontSize: '16px', color: '#014988' }}>
                      Last Name
                ...
                <TableBody>
                  {row.user.map((historyRow) => (
                    <TableRow key={historyRow.date}>
                      <TableCell align="center">{historyRow.Fname}</TableCell>
                      <TableCell align="center">{historyRow.Lname}</TableCell>
                      <TableCell align="center">{historyRow.country}</TableCell>
                      <TableCell align="center">{historyRow.dob.split('T')[0]}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
      ...
      <Table aria-label="collapsible table">
        <TableHead>
          <TableRow>
            ...
            <TableCell align="center" style={{ fontSize: '18px', color: '#014988' }}>
              User&nbsp;Since
            </TableCell>
            <TableCell align="center" style={{ fontSize: '18px', color: '#014988' }}>
              Current&nbsp;Status
